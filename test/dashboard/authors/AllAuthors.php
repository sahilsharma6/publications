...
<head>
    ...
    <style>
        ...
        @media (max-width: 720px) {
            ...
            .summary-strip .summary-card:last-child {
                display: none;
            }
        }
                ...
                <div class="toolbar-left">
                    <div class="search-box">
                        ...
                        <input type="text" class="search-input" name="search" id="searchInput"
                            value="<?= htmlspecialchars($search, ENT_QUOTES) ?>" placeholder="Search authors…"
                            autocomplete="off">
                    </div>
                ...
                <?php foreach ($authors as $i => $a):
                    ...
                    <div class="grid-card" style="animation-delay:<?= $i * .04 ?>s">
                        ...
                        <div class="grid-avatar-wrap">
                            <?php if ($hasImg): ?>
                                <img src="<?= $imgPath ?>" class="grid-avatar"
                            ...
                            <?php else: ?>
                                <?php foreach (array_slice($books, 0, 2) as $bk): ?>
                                    <span class="book-chip"><i
                                            class='bx bx-book-alt'></i><?= htmlspecialchars($bk, ENT_QUOTES) ?></span>
                                <?php endforeach; ?>
                                <?php if (count($books) > 2): ?>
                                    <span class="book-chip"
                                        style="background:var(--gray-100);color:var(--gray-500);border-color:var(--gray-200)">
                                        +<?= count($books) - 2 ?> more
                                    </span>
                                <?php endif; ?>
                            <?php endif; ?>
                                            ...
                                            <div class="detail-body">
                                                ...
                                                <?php if ($a['description']): ?>
                                                    <div class="detail-desc">
                                                        <?= nl2br(htmlspecialchars($a['description'], ENT_QUOTES)) ?></div>
                                                <?php endif; ?>
                                                <?php if (!empty($books)): ?>
        ...
        <?php if ($totalRows > 0): ?>
            <div class="pagination-row">
                ...
                <?php if ($totalPages > 1):
                    ...
                    ?>
                    <ul class="pag-list">
                        <li><a class="pag-btn <?= $currentPage <= 1 ? 'is-disabled' : '' ?>"
                                href="<?= pageUrl(['page' => 1]) ?>"><i class='bx bx-chevrons-left'></i></a></li>
                        <li><a class="pag-btn <?= $currentPage <= 1 ? 'is-disabled' : '' ?>"
                                href="<?= pageUrl(['page' => $currentPage - 1]) ?>"><i class='bx bx-chevron-left'></i></a></li>
                        ...
                        <?php endif; ?>
                        <?php for ($p = $pS; $p <= $pE; $p++): ?>
                            <li><a class="pag-btn <?= $p === $currentPage ? 'is-active' : '' ?>"
                                    href="<?= pageUrl(['page' => $p]) ?>"><?= $p ?></a></li>
                        <?php endfor; ?>
                        <?php if ($pE < $totalPages): ?>
                            <?php if ($pE < $totalPages - 1): ?>
                                <li><span class="pag-ellipsis">…</span></li><?php endif; ?>
                            <li><a class="pag-btn" href="<?= pageUrl(['page' => $totalPages]) ?>"><?= $totalPages ?></a></li>
                        <?php endif; ?>

                        <li><a class="pag-btn <?= $currentPage >= $totalPages ? 'is-disabled' : '' ?>"
                                href="<?= pageUrl(['page' => $currentPage + 1]) ?>"><i class='bx bx-chevron-right'></i></a></li>
                        <li><a class="pag-btn <?= $currentPage >= $totalPages ? 'is-disabled' : '' ?>"
                                href="<?= pageUrl(['page' => $totalPages]) ?>"><i class='bx bx-chevrons-right'></i></a></li>
                    </ul>
                <?php endif; ?>
            </div>
        <?php endif; ?>
